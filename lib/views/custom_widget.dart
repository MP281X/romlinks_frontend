    ...
    return GetX<ButtonWController>(
      ...
      builder: (controller) {
        return GestureDetector(
          ...
          child: ContainerW(
            FittedBox(
              child: (!controller.animation.value) ? TextW(text1, maxLine: 1) : const Center(child: CircularProgressIndicator(color: ThemeApp.primaryColor)),
            ),
            padding: padding,
            margin: margin,
            color: color,
...
//! custom snackbar
Future<void> snackbarW(String title, String? msg) async {
...
class ScaffoldW extends StatelessWidget {
  ScaffoldW(this.child, {this.scroll = false, this.auth = false});
